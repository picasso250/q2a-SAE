...
*/
	{
		...
		if (strlen($php_errormsg)) {
			...
			file_put_contents('saestor://log/'.date('Y-m-d H:i:s ').microtime().'.txt', 
			// @error_log(
				'PHP Question2Answer more info: '.$php_errormsg." in eval()'d code from ".qa_html($filename));
		}
...
*/
	{
		...
		file_put_contents('saestor://log/'.date('Y-m-d H:i:s ').microtime().'.txt', 
		// @error_log(
			'PHP Question2Answer fatal error: '.$message);
